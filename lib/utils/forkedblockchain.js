...
const ForkedStorageTrie = require("./forkedstoragetrie.js");
...
ForkedBlockchain.prototype.createGenesisBlock = function(callback) {
  const self = this;
  let blockNumber = this.forkBlockNumber || "latest";
    ...
    self.web3.eth.getTransactionReceipt(hash, function(err, receiptJson) {
      ...
      async.parallel(
        ...
        function(err, result) {
          ...
          const logs = receiptJson.logs.map(function(log) {
        ...
        self.getBlock(number, function(err, block) {
          ...
          self.web3.currentProvider.send(
            ...
            function(err, res) {
              ...
              const logs = res.result.map(function(log) {
