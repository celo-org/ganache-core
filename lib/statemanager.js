...
StateManager.prototype._setTransactionDefaults = function(tx, isTransaction) {
  if (isTransaction && (!tx.gasLimit || tx.gasLimit.length === 0)) {
    tx.gasLimit = utils.toBuffer(this.blockchain.defaultTransactionGasLimit);
  }

  if (!tx.gasPrice || tx.gasPrice.length === 0) {
    tx.gasPrice = this.gasPrice(tx.feeCurrency);
  }

  if (!tx.value || tx.value.length === 0) {
    tx.value = Buffer.from([0]);
  }

  if (!tx.to || tx.to.length === 0 || tx.to.equals(ZERO_BUFFER)) {
...
StateManager.prototype.signTx = function(txData, address) {
  ...
  tx.feeCurrency = txData.feeCurrency; // Transaction from EthereumJs extended
  this._setTransactionDefaults(tx, true);

  tx.sign(Buffer.from(account.secretKey));
  return {
    raw: utils.addHexPrefix(tx.serialize().toString("hex")),
    tx: tx.encode()
  };
};
