...
const BlockchainDouble = require("./blockchain_double.js");
const ForkedBlockchain = require("./utils/forkedblockchain.js");
...
const ECIES = require("@celo/utils").ECIES;

const ZERO_BUFFER = Buffer.from([0]);

const to = require("./utils/to");
const random = require("./utils/random");
const TXRejectedError = require("./utils/txrejectederror");
...
StateManager.prototype.initialize = function(callback) {
  ...
  if (self.options.accounts) {
    ...
    for (let i = 0; i < self.total_accounts; i++) {
      accounts.push(
        self.createAccount({
          index: i,
...
StateManager.prototype.createAccount = function(opts, i) {
  ...
  if (opts.generate) {
    ...
    const index = typeof opts.index === "undefined" ? i : opts.index;
    const acct = this.wallet.derivePath(this.wallet_hdpath + index); // index is a number
    secretKey = acct.getWallet().getPrivateKey(); // Buffer
  }

  const publicKey = utils.privateToPublic(secretKey);
...
StateManager.prototype.processNextAction = function(override) {
  ...
  const queued = this.action_queue.shift();
...
StateManager.prototype.sign = function(address, dataToSign) {
  ...
  const msg = to.buffer(dataToSign, "hex");
  const msgHash = utils.hashPersonalMessage(msg);
  const sgn = utils.ecsign(msgHash, Buffer.from(secretKey));
  ...
  self.blockchain.processNextBlock(timestamp, function(runtimeError, transactions, vmOutput) {
    ...
    async.eachSeries(
      ...
      function(tx, finishedPrinting) {
        const hash = to.hex(tx.hash());
        const error = runtimeError == null ? { results: {} } : runtimeError;
...
StateManager.prototype.getLogs = function(filter, callback) {
  ...
  let expectedAddress = filter.address && (Array.isArray(filter.address) ? filter.address : [filter.address]);
  expectedAddress =
    expectedAddress &&
    expectedAddress.map(function(a) {
      return a.toLowerCase();
    });
  const expectedTopics = filter.topics || [];
  ...
  async.parallel(
    ...
    function(err, results) {
      ...
      const fromBlock = results.fromBlock;
      let toBlock = results.toBlock;
      const latestBlock = results.latestBlock;
          ...
          self.blockchain.getBlockLogs(current, function(err, blockLogs) {
            ...
            filtered = filtered.filter(function(log) {
              ...
              for (let i = 0; i < expectedTopics.length; i++) {
                ...
                const isMatch = Array.isArray(expectedTopic)
...
StateManager.prototype.revert = function(snapshotId, callback) {
  ...
  const timeAdjustment = this.snapshots[snapshotId].timeAdjustment;
