...
var getPredeployedContracts = require("./predeployedContracts.js");
    ...
    self.latestBlock(function(err, block) {
      ...
      self.createGenesisBlock(function(err, block) {
        ...
        self.vm.stateManager.checkpoint(() => {
          // Deploy Celo predeployed contracts.
          const predeployedContracts = getPredeployedContracts(accounts[0].address);
          async.eachSeries(
            predeployedContracts,
            function(predeployedContract, finished) {
              self.putContractCode(predeployedContract.code, predeployedContract.address, finished);
              async.eachSeries(predeployedContract.storage, function(kv, finished) {
                self.putContractStorage(kv.key, kv.value, predeployedContract.address, finished);
              });
            },
            function(err) {
              if (err) {
...
BlockchainDouble.prototype.putContractStorage = function(key, value, address, callback) {
  var self = this;
  ...
  this.vm.stateManager.putContractStorage(address, key, value, function(err) {
    if (err) {
      return callback(err);
    }
...
BlockchainDouble.prototype.putContractCode = function(value, address, callback) {
  var self = this;

  value = utils.toBuffer(value);
  address = utils.toBuffer(address);

  this.vm.stateManager.putContractCode(address, value, function(err) {
    if (err) {
      return callback(err);
    }
...
BlockchainDouble.prototype.createBlock = function(parent, callback) {
  ...
  getParent(function(err, parent) {
    ...
    if (self.accounts.length > 0) {
      var account = self.accounts[(parentNumber + 1) % self.accounts.length];
      block.header.coinbase = account.address;
      const { r, s, v } = utils.ecsign(block.header.parentHash, account.secretKey);
      var vBuf = Buffer.alloc(1);
      vBuf[0] = v;
      block.header.signature = Buffer.concat([vBuf, r, s]);
    }
