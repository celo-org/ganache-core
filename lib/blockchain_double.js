...
const estimateGas = require("./utils/gas/estimateGas");
...
BlockchainDouble.prototype.popBlock = function(callback) {
  ...
  this.data.blocks.last(function(err, block) {
    ...
    const requests = [];
    const blockHash = to.hex(block.hash());
...
BlockchainDouble.prototype.createBlock = function(parent, emulateParent, callback) {
  ...
  getParent(function(err, parent) {
    ...
    if (parent != null && emulateParent) {
      ...
      const parentNumber = parent != null ? to.number(parent.header.number) : -1;
...
BlockchainDouble.prototype.processBlock = async function(vm, block, commit, callback) {
  ...
  for (let v = 0; v < results.receipts.length; v++) {
    const result = results.results[v];
    const receipt = results.receipts[v];
    const tx = block.transactions[v];
    ...
    if (result.execResult.exceptionError === undefined) {
      for (let i = 0; i < receipt.logs.length; i++) {
        ...
        const address = to.hex(receiptLog[0]);
        const topics = [];

        for (let j = 0; j < receiptLog[1].length; j++) {
          topics.push(to.hex(receiptLog[1][j]));
        }

        const data = to.hex(receiptLog[2]);
...
BlockchainDouble.prototype.processNextBlock = function(timestamp, callback) {
  ...
  let totalGasLimit = 0;
  const maxGasLimit = to.number(self.blockGasLimit);

  while (self.pending_transactions.length > 0) {
    const tx = self.pending_transactions[0];
    const gasLimit = to.number(tx.gasLimit);
  ...
  if (currentTransactions.length === 0 && self.pending_transactions.length > 0) {
    // Error like geth.
    const error = "Unexpected error condition: next transaction exceeds block gas limit";
...
BlockchainDouble.prototype.processTransactionTrace = async function(hash, params, callback) {
  ...
  this.getTransactionReceipt(targetHash, function(err, receipt) {
    ...
    self.getBlock(targetBlock.header.parentHash, function(err, parent) {
      ...
      const stateTrie = self.createStateTrie(self.data.trie_db, parent.header.stateRoot, {
      ...
      self.createBlock(parent, false, function(err, block) {
        ...
        for (let i = 0; i < targetBlock.transactions.length; i++) {
          const tx = targetBlock.transactions[i];
